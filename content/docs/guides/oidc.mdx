...
To use the generic provider, you utilize the [`OidcProvider`](https://github.com/marcjazz/authkestra/tree/0.1.1/authkestra-oidc/src/provider.rs) struct from the `authkestra-oidc` crate.
...
For a full working example using Axum and environment variables, check out the [`oidc_generic.rs`](https://github.com/marcjazz/authkestra/tree/0.1.1/authkestra-examples/src/bin/oidc_generic.rs) file in the `authkestra-examples` crate.
