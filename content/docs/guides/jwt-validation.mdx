...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cache = JwksCache::new(jwks_uri, refresh_interval);
...
For a full implementation example, including error handling and async integration, see the [`actix_resource_server.rs`](https://github.com/marcjazz/authkestra/tree/0.1.1/authkestra-examples/src/bin/actix_resource_server.rs) file in the `authkestra-examples` crate on GitHub.
