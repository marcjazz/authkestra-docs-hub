...
Authkestra uses Axum's state management to share authentication logic across handlers. Define an `AppState` and implement `FromRef` for `Authkestra` or use `AuthkestraState` directly.
...
```rust
let authkestra_state = AuthkestraState { authkestra };
