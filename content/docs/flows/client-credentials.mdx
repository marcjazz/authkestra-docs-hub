...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client_id = std::env::var("AUTHKESTRA_CLIENT_ID")
        .expect("AUTHKESTRA_CLIENT_ID must be set");
    let client_secret = std::env::var("AUTHKESTRA_CLIENT_SECRET")
        .expect("AUTHKESTRA_CLIENT_SECRET must be set");
    let token_url = std::env::var("AUTHKESTRA_TOKEN_URL")
